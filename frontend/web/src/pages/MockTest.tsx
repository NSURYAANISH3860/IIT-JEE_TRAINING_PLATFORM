...
function MockTest({ dashboardMode = false }: { dashboardMode?: boolean }) {
  ...
  if (dashboardMode) {
    const filteredDashboardMockTests = dashboardMockTests.filter((test) => {
        ...
        <section className="dashboard-mock-catalog">
          <div className="dashboard-mock-filters" aria-label="Mock test filters">
            {dashboardFilters.map((filter, index) => (
              <button
                className={activeDashboardFilter === filter || (index === 0 && activeDashboardFilter === "All") ? "active" : ""}
                key={filter}
                type="button"
          ...
          {analysisTitle && (
            <div className="dashboard-mock-analysis-banner">
              Analysis opened for {analysisTitle}. Review score, rank, accuracy, and time from the test card.
            </div>
          )}

          <div className="dashboard-mock-list">
            {filteredDashboardMockTests.map((test) => {
